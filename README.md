...
A Telegram bot framework powered by Claude Code CLI and a three-tier agent architecture. It manages code, runs commands, and handles complex tasks through an intelligent agent pipeline.

## How It Works

```
You (Telegram) → Chat Agent → Claude Code CLI → Your Codebase
```
...
- **Web chat interface** — talk to your bot from the browser
